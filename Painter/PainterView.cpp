...
BEGIN_MESSAGE_MAP(CPainterView, CScrollView)
	...
	ON_COMMAND(ID_EDIT_ADDSHAPE_MYPOLYGONFIGURE, OnEditAddshapeMyPolygonFigure)
...
void CPainterView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	{
		case OP_SPLINE:
			//if (((CSpline*)pDoc->m_ShapesList.GetTail())->m_PointsArray.GetSize()>=2)
			//	((CSpline*)pDoc->m_ShapesList.GetTail())->m_PointsArray.Add(CPoint((LogPoint.x - ((CSpline*)pDoc->m_ShapesList.GetTail())->m_PointsArray[((CSpline*)pDoc->m_ShapesList.GetTail())->m_PointsArray.GetSize() - 1].x)/2, (LogPoint.y - ((CSpline*)pDoc->m_ShapesList.GetTail())->m_PointsArray[((CSpline*)pDoc->m_ShapesList.GetTail())->m_PointsArray.GetSize() - 1].y)/2));
			((CSpline*)pDoc->m_ShapesList.GetTail())->m_PointsArray.Add(LogPoint);
...
void CPainterView::OnEditAddshapeMyPolygonFigure() 
{
	m_CurOper=OP_MYPOLYGONFIGURE;
	::SetClassLong(GetSafeHwnd(), GCL_HCURSOR, (LONG)m_hcurPolygon);
}
